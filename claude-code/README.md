...
```
project-root/
├── CLAUDE.md                 # bộ não trung tâm — import contexts + rules via @path
└── .claude/
    ├── settings.json         # model, permissions, hooks, env, mcpServers
    ├── agents/               # sub-agent: mỗi file = 1 role chuyên biệt
    │   ├── builder.md        # surgical edit 1-2 file
    │   ├── investigator.md   # read-only locator (haiku)
    │   ├── reviewer.md       # PR/diff reviewer
    │   ├── architect.md      # system design, không viết code (opus)
    │   ├── debugger.md       # root cause analysis
    │   └── tester.md         # unit + integration test writer
    ├── commands/             # slash command custom (/foo)
    │   ├── commit.md         # /commit
    │   ├── review.md         # /review [PR#]
    │   ├── ship.md           # /ship — lint→test→commit→PR
    │   └── test.md           # /test [--unit|--e2e|--all]
    ├── contexts/             # context theo domain, import vào CLAUDE.md
    │   ├── api.md            # NestJS conventions
    │   ├── web.md            # Next.js App Router conventions
    │   └── db.md             # Prisma/Postgres conventions
    ├── hooks/                # shell hook (PreToolUse, PostToolUse, SessionStart…)
    │   ├── session-start.sh  # in branch/status khi mở session
    │   ├── pre-tool-bash.sh  # chặn lệnh nguy hiểm
    │   └── post-tool-edit.sh # chạy ESLint sau mỗi lần edit
    ├── mcp/
    │   └── README.md         # tài liệu các MCP server đã cấu hình
    ├── memory/               # memory persistent across session
    │   ├── MEMORY.md         # index
    │   ├── user_role.md
    │   ├── feedback_*.md
    │   └── project_status.md
    ├── plugins/              # skill mở rộng dạng generator/scaffold
    │   ├── api-scaffold/     # generate NestJS module
    │   └── component-gen/    # generate React component
    ├── rules/                # rule theo domain, import vào CLAUDE.md
    │   ├── api.md            # NestJS-specific rules
    │   ├── security.md       # input validation, secrets, SQL injection
    │   └── git.md            # branching, commit, PR workflow
    └── skills/               # skill invoke on-demand
        ├── migrate-db/       # safe Prisma migration (3-step)
        ├── deploy/           # staging + prod deploy
        └── pr-review/        # full PR review pipeline
```

Xem full example tại [`examples/`](./examples/).
